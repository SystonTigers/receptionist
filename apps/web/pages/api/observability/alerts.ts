...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL ?? ''}/observability/alerts`, {
      headers: {
    ...
    if (!response.ok) {
      req.logger?.warn('Observability alerts upstream error', {
    ...
    req.logger?.error('Observability alerts proxy failed', { error: normalizeError(error) });

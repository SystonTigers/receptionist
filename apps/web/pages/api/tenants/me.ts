...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL ?? ''}/tenants/me`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: payload.error ?? 'Unable to fetch tenant' });
    }
